...
switch ($method) {
    case 'GET':
        if (isset($_GET['id'])) {
            $id = $_GET['id'];
            $stmt = $conn->prepare("SELECT \"Id\", \"IdReuniao\", \"Descricao\", \"ValorDespesa\", repasse, compra_literatura FROM despesas WHERE \"Id\" = ?");
            $stmt->execute([$id]);
            $despesa = $stmt->fetch();
    ...
    case 'POST':
        ...
        try {
            $stmt = $conn->prepare(
                "INSERT INTO despesas (\"IdReuniao\", \"Descricao\", \"ValorDespesa\", repasse, compra_literatura)
            ...
            if ($stmt->execute([$data['IdReuniao'], $data['Descricao'], $data['ValorDespesa'], $repasse, $compra_literatura])) {
                $id = $stmt->fetchColumn();
